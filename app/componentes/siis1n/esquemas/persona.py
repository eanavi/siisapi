...
class Direccion(BaseModel):
    zona: str = Field(..., example="Villa Dolores",
                      title="Zona", description="Zona de la direccion")
    calle: str = Field(..., example="Av. 6 de Agosto",
                       title="Calle", description="Calle de la direccion")
    numero: str = Field(..., example="123", title="Numero",
                        description="Numero de la direccion")
    ciudad: str = Field(..., example="La Paz", title="Ciudad",
                        description="Ciudad de la direccion")


class DireccionTipo(BaseModel):
    tipo: str = Field(..., example="personal",
                      title="Tipo de Direccion", description="Tipo de direccion")
    direccion: Direccion = Field(..., title="Direccion",
                                 description="Direccion de la persona")


class PersonaBase(BaseModel):
    tipo: str = Field(..., title="Tipo de persona",
                      description="Tipo de persona: 'E' = Empleado, 'P' = Paciente")
    ci: Optional[str] = Field(None, example="548732", title="Carnet de Identidad",
                              description="Numero de carnet de identidad")
    paterno: Optional[str] = Field(
        None, example="Fernández", title="Apellido Paterno", description="Apellido paterno de la persona")
    materno: Optional[str] = Field(
        None, example="Pérez", title="Apellido Materno", description="Apellido materno de la persona")
    nombres: str = Field(..., example="Juan Antonio",
                         title="Nombres", description="Nombres de la persona")
    fecha_nacimiento: date = Field(..., example="1990-01-01", title="Fecha de Nacimiento",
                                   description="Fecha de nacimiento de la persona")
    sexo: str = Field(..., example="M", title="Sexo",
...
class PersonaResponse(BaseModel):
    ...
    tipo: str = Field(..., example="E", title="Tipo de persona",
                      description="Tipo de persona: 'E' = Empleado, 'P' = Paciente")
    ci: Optional[str] = Field(None, example="548732", title="Carnet de Identidad",
                              description="Numero de carnet de identidad")
    paterno: Optional[str] = Field(
